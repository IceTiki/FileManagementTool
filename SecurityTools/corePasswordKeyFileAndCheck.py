...
def creatNewKey():
    # 说明
    print('='*64+'\n使用scrypt进行硬内存哈希生成密钥\n参数:N=2^20, salt=\'Tiki_Essential.Security\'(utf-8), r=8, p=1, buflen=64')
    # 初始化
    var_N = 2**20
    salt = 'Tiki_Essential.Security'
    password = input('='*64+'\n请输入 密码\n>')
    # 内存检查
    ...
    print('='*64+'\n计算时间%fs\n参数:r=8,p=1,buflen=64,N=%d,sale=\'Tiki_Essential.Security\'(utf-8)\n密钥:     %s\n密码验证字符串:     %s' %
...
class cmd:
    ...
    def gene():
        ...
        while 1:
            i_str = input(
                '='*64+'\n是否请选择密钥输出模式(T:命令行输出,F:文件输出,E:生成密钥文件,C:生成密钥验证文件)\n>')
