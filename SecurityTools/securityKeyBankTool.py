...
class cmd:
    ...
    def init_core():
        # 检查文件
        if os.path.isfile('Core_KeyBank.yml'):
            print('Core_KeyBank.yml已经存在，请检查删除后进行创建')
            return

        # 输入核心密钥
        coreKey = GT.getCoreKey()

        GT.encrypto(coreKey, 'Core_KeyBank.yml', 'banks: []', True)

        repo = git.Repo.init('.')
        repo.index.add(['Core_KeyBank.yml'])
    ...
    def init_bank():
        # 读取核心密码库
        if not os.path.isfile('Core_KeyBank.yml'):
            print('请先创建核心密码库')
            return
        coreKey = GT.getCoreKey()
        try:
            text = GT.decrypto(coreKey, 'Core_KeyBank.yml')
        ...
        while 1:
            ...
            if not re.match('^[A-Z][A-Z0-9]*$', bankName):
                print('密码库名格式错误')
            elif os.path.isfile('%s_KeyBank.yml' % bankName):
                print('密码库已存在')
                return
            else:
                break
        ...
        text = GT.yaml(dict_)
        # 写入核心密码库
        GT.encrypto(coreKey, 'Core_KeyBank.yml', text)
        # 创建密码库
        ...
        GT.encrypto(bankKey, '%s_KeyBank.yml' %
                    bankName, init_bank_str)

        # git
        repo = git.Repo.init('.')
        repo.index.add(['Core_KeyBank.yml', '%s_KeyBank.yml' % bankName])
    ...
    def update_bank():
        bankName = input('请输入密码库名\n>')
        if not os.path.isfile('%s_KeyBank.yml' % bankName):
            print('密码库不存在')
            return
        bankKey = input('请输入密码库密码\n>')
        ...
        # 解密文件
        try:
            GT.decrypto(bankKey, '%s_KeyBank.yml' % bankName, writeIn=True)
        except Exception as e:
            print('解密失败:%s' % e)
            return
        ...
        # 加密文件
        GT.encrypto(bankKey, '%s_KeyBank.yml' % bankName, writeIn=True)

        # git
        repo = git.Repo.init('.')
        repo.index.add(['%s_KeyBank.yml' % bankName])
    ...
    def delete_bank():
        ...
        GT.encrypto(coreKey, 'Core_KeyBank.yml', text)
        # 删除密码库
        os.remove('%s_KeyBank.yml' % bankName)
