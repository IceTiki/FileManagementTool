...
class FolderStatus:
    def __init__(self, folder_path=".\\") -> None:
        ...
        self.folders_data = [
            {"abspath": path, "relpath": os.path.relpath(path, self.folder_path)}
    ...
    def formatted_status(self):
        ...
        file_status = [
            "|-|".join([file["relpath"], file["sha256"], str(file["size"])])
            for file in self.files_data
        ]
    ...
    def update_data(
        self, data_dir: str = None, history_dir: str = None, only_scan_and_print=False
    ):
        """
        更新yaml文件中的数据
...
class AutoUpdate:
    ...
    def alpha_update(self):
        ...
        new_idx_name = os.path.join(os.path.dirname(self.idx_path), new_idx_name)
...
====================help===================="""
    ...
    def fm_init(self, content):
        ...
        fmo_exid = str().join(random.choices("0123456789abcdef", k=128))
...
status_id: '{fmo_exid}' # 对象状态识别号——128位16进制(小写字母)随机字符串，在创建/更新对象时随机生成"""
        ...
        shutil.copy2(
            ...
            os.path.join(fmo_inx_dir, "tag_list.yml"),
        )
        shutil.copy2(
            self.repository_assets(
        ...
        fs.update_data(
            data_dir=os.path.join(fmo_inx_dir, "status.yml"),
