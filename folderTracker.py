...
class FolderStatus:
    ...
    def FolderStatus(self):
        ...
        self.folderData = {'FolderStatus': {
            'Info': Info, 'FolderList': self.folderPath_List, 'FileHashList': self.fileHash_List}}
        log('数据整理为字典完成')
    ...
    def logHistory(self):
        if os.path.isfile(self.yamlPath):
            ...
            changesInfo = {'RootPath': os.path.abspath(self.path), 'Generated_time': time.strftime(
            ...
            self.folderData['HistoryChanges'] = [{'ChangesInfo': {'Generated_time': time.strftime(
                "%Y-%m-%d %H:%M:%S", time.localtime()), 'Summary': "initialize."}}]
